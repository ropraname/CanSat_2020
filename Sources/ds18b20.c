...
void ds18b20match(volatile uint8_t *port, volatile uint8_t *direction,
		volatile uint8_t *portin, uint8_t mask, uint8_t *rom) {
...
uint8_t ds18b20convert(volatile uint8_t *port, volatile uint8_t *direction,
		volatile uint8_t *portin, uint8_t mask, uint8_t *rom) {
...
uint8_t ds18b20rsp(volatile uint8_t *port, volatile uint8_t *direction,
		volatile uint8_t *portin, uint8_t mask, uint8_t *rom, uint8_t *sp) {
...
uint8_t ds18b20wsp(volatile uint8_t *port, volatile uint8_t *direction,
		volatile uint8_t *portin, uint8_t mask, uint8_t *rom, uint8_t th,
...
uint8_t ds18b20csp(volatile uint8_t *port, volatile uint8_t *direction,
		volatile uint8_t *portin, uint8_t mask, uint8_t *rom) {
...
//! Read temperature
uint8_t ds18b20read(volatile uint8_t *port, volatile uint8_t *direction,
		volatile uint8_t *portin, uint8_t mask, uint8_t *rom,
		int16_t *temperature) {
...
//! Read ROM address
uint8_t ds18b20rom(volatile uint8_t *port, volatile uint8_t *direction,
		volatile uint8_t *portin, uint8_t mask, uint8_t *rom) {
	//Read DS18B20 rom
	...
	//Pull-up check
	if ((rom[0] | rom[1] | rom[2] | rom[3] | rom[4] | rom[5] | rom[6] | rom[7])
			== 0)
		return DS18B20_ERROR_PULL;
...
uint8_t ds18b20read_temp(int16_t *temp) {
	// convert without ROM match
	ds18b20convert(&PORTB, &DDRB, &PINB, (1 << PB7), NULL);
	ds18b20wsp(&PORTB, &DDRB, &PINB, (1 << PB7), NULL, NULL, NULL,
	DS18B20_RES10);
	ds18b20convert(&PORTB, &DDRB, &PINB, (1 << PB7), NULL);
	ds18b20read(&PORTB, &DDRB, &PINB, (1 << PB7), NULL, temp);
	return 1;
}
